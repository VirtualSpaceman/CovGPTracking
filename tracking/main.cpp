...
int main(int argc, char** argv)
{
    ...
    */
    ...
    deque<Point2d> memo_track;
    Point2d p_aux, p_teste;
    ...
    for(;;)
    {
        ...
        if(init)
        {
            ...
            roi = Rect2d(328.00,189.00,53.00,50.00);
        ...
        if(!track_feat.empty())
        {
            ...
            if(memo_track.size() >= Nt){
                double predy = gpy.get_means({count})[0];
                double predx = gpx.get_means({count})[0];
                p_teste = Point2d(predx, predy);
            ...
            if(memo_track.size() >= Nt){
                memo_track.pop_front();
                memo_track.push_back((p_aux+p_teste)/2);
                cout << "Predict: " <<(p_aux+p_teste)/2 << '\n';
                r_track = Rect(((p_aux+p_teste)/2), r_track.width, r_track.height);
            }
